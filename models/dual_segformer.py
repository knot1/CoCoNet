...
def __init_weight(feature, conv_init, norm_layer, bn_eps, bn_momentum, **kwargs):
    for _, module_layer in feature.named_modules():
        if isinstance(module_layer, (nn.Conv1d, nn.Conv2d, nn.Conv3d)):
            conv_init(module_layer.weight, **kwargs)
        elif isinstance(module_layer, norm_layer):
            module_layer.eps = bn_eps
            module_layer.momentum = bn_momentum
            nn.init.constant_(module_layer.weight, 1)
            nn.init.constant_(module_layer.bias, 0)
